...
def getTemplateValue(template):
    try:
        template_path = os.path.join(templates_base, f"{template}.json")
        with open(template_path) as json_file:
            return random.choice(json.load(json_file)[template])
    except IOError as e:
        process.outputMessage(f"Error opening template file {template_path}: {e}", "error")
        exit()
    except Exception as e:
        process.outputMessage(f"An error occurred for {template_path}: {e}", "error")
        return False
...
def getOutputPath(type, item_id, ext):
    now = datetime.datetime.now()
    base_dir=os.path.join(os.getcwd(),"outputs")
    date_depth="%Y/%m/%d"#/%H/%M/%S"

    file_path = os.path.join(base_dir, now.strftime(date_depth),type, f"{item_id}.{ext}")
...
def generateObject(object_prompt, object_prompt_list):
    ...
    try:
        ...
        process.outputMessage(f"Error generating object : {e}", "error")
    ...
    try:
        ...
        process.outputMessage(f"Error parsing object completion: {completion}","error")
        return False

    # use the process_id for the media_id
    media_id = process.process_id
    ...
    if "title" in completion and "tagline" in completion and "description" in completion:
        ...
        process.outputMessage(f"Error generating object, missing important details (title, tagline or description) in completion.","error")
        return False
...
def generateImagePrompt(media_object):
    ...
    if args.verbose: process.outputMessage(f"Prompt\n {full_prompt}","verbose")
...
def generateImage(image_completion, media_object):
    ...
    for _ in range(retries):
        try:
            ...
            process.outputMessage(f"Attempt {_+1} of {retries} failed to generate image for '{media_object['title']}'.","warning")
            if args.verbose: print(e)
            continue
    else:
        process.outputMessage(f"Error generating image for '{media_object['title']}' after {retries} attempts","error")
        return False
...
def createDirectory(directory):
    ...
    try:
        ...
        process.outputMessage(f"Error creating directory {directory}.","error")
        return False
...
def saveItem(item, type, item_id):
    ...
    if result == True:
        try:
            ...
            process.outputMessage(f"Error saving {type}.\n{e}","error")
            return False
    else:
        return False
...
def main():
    ...
    while i < generate_count:
        ...
        media_object=generateObject(object_prompt, object_prompt_list)
        if not media_object:
            continue
        else:
            media_object = json.loads(media_object)
            if args.verbose:
